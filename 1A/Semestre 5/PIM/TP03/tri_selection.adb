...
procedure Tri_Selection is
    ...
    -- Objectif : Trier le tableau.
    --    Le tableau est trier part sélection, le plus petit element est trouvé et placé en premier.
    -- Paramètres :
    --    Tab : le tableau à afficher.

    function Trier(Tab: in out T_Tableau) return T_Tableau is
    index_min: Integer;  -- index du minimum des éléments du tableau
    Copie : Integer;
    Min : Integer;
    begin
        for etape in 1..Tab.Taille-1 loop
            Min := Tab.Elements(etape);
            index_min := etape;
            for J in etape + 1..Tab.Taille loop
                if Tab.Elements(J) < Min then
                    index_min := J;
                    Min := Tab.Elements(index_min);
                else
                    null;
                end if;
            end loop;
            Copie := Tab.Elements(etape);
            Tab.Elements(etape) := Tab.Elements(index_min);
            Tab.Elements(index_min) := Copie;
        end loop;
        return Tab;
    end Trier;
