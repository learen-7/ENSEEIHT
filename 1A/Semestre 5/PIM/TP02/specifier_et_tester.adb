...
procedure Specifier_Et_Tester is
    ...
    function Pgcd (A, B : in Integer) return Integer with
        Pre =>  A > 0 and B > 0,
        Post =>  Pgcd'Result >= 1 -- Pgcd'result signifie le résultat de la fonction Pgcd
    ...
    -- Cette procedure est fausse, en effet la variable A de la fonction pgcd
    -- doit être >0 et dans la procédure A=0
    -- son execution ne renvoie rien, le programme fait une boucle infinie
    ...
    procedure Permuter (A, B: in out Integer) with
            Post => A = B'Old and B = A'Old -- B'Old et A'Old signifie les anciennes
                                            -- valeurs de B et A, les valeurs
                                            -- qu'ils avaient avant de passer
                                            -- dans la procédure Permuter
    is
        Copie_A : Integer;
    begin
        Copie_A := A;
