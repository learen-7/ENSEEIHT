...
procedure Evaluer_Alea is
    ...
    begin
        ...
        Max := Tab.Elements(1);
        Moyenne := 0.0;
        for indice in 1..Borne loop
            if Tab.Elements(indice) < Min then
                Min := Tab.Elements(indice);
            elsif Tab.Elements(indice) > Max then
                Max := Tab.Elements(indice);
            else
                null;
            end if;
            Moyenne := Moyenne + Float(Tab.Elements(indice)) * Float(indice);
        end loop;
