...
procedure Puissance is
    ...
    --    Exposant: l'exposant
    --
    -- Précondition : nothing
    function Puissance_Iteratif (Nombre: in Float ; Exposant : in Integer) return Float
    is
        Result : Float;
    begin
        if Exposant < 0 then
            Result := 1.0 / Puissance_Positive_Iteratif(Nombre, - Exposant);
        else
    ...
    begin
        if Exposant = 0 then
            ...
            if Exposant > 0 then
                ...
                return 1.0 / (Nombre) * Puissance_Recursif(Nombre, Exposant + 1);
            end if;
        end if;
